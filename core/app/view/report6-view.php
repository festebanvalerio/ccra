<?php
    ...
    if ($indicador == 0) {
        ...
        for ($i = $sd; $i <= $ed; $i += (60 * 60 * 24)) {
            $lstPago[$i] = PedidoData::getTipoDescuentoGroupByFecha($_SESSION["sede"], date("Y-m-d", $i), date("Y-m-d", $i));
        }        
    } else if ($indicador == 1) {
        for ($i = $mesInicio; $i <= $mesFin; $i++) {
            $lstPago[$i] = PedidoData::getTipoDescuentoGroupByMes($_SESSION["sede"], $anho, $i);
        }
    } else if ($indicador == 2) {
        for ($i = $anhoInicio; $i <= $anhoFin; $i++) {
            $lstPago[$i] = PedidoData::getTipoDescuentoGroupByAnho($_SESSION["sede"], $i);
        }
    }    
    $lstTipo = ParametroData::getAll(1, "TIPO REPORTE");
    $lstAnho = ParametroData::getAllAnho();
    $lstMes = ParametroData::getAllMes();
    $lstTipoDescuento = ParametroData::getAll(1, "TIPO DESCUENTO");
?>
	...
	<div class="row">
		<div class="col-md-12">
			<div class="panel panel-primary">
    			<div class="panel-heading" style="background-color: #3c8dbc;"><h3 class="panel-title">Reporte Tipo Descuento</h3></div>
    			<div class="panel-body">
    				<form class="form-horizontal" method="post" id="report6" action="" role="form" autocomplete="off">    			
            			<div class="form-group">
        					...
        					<div class="col-md-4 col-sm-12" style="padding-top: 25px;">
        						...
        						<?php if ($indicador == 0) { ?>
        						<a class="btn btn-primary" id="btnExportar" href="excelReporte.php?sede=<?php echo $_SESSION["sede"]; ?>&fechaInicio=<?php echo $fechaInicio; ?>&fechaFin=<?php echo $fechaFin; ?>&opcion=6&indicador=<?php echo $indicador; ?>" role="button" title="Descargar" target="_blank"><em class="fa fa-download"></em></a>
        						<?php } else if ($indicador == 1) { ?>
        						<a class="btn btn-primary" id="btnExportar" href="excelReporte.php?sede=<?php echo $_SESSION["sede"]; ?>&anho=<?php echo $anho; ?>&mesInicio=<?php echo $mesInicio; ?>&mesFin=<?php echo $mesFin; ?>&opcion=6&indicador=<?php echo $indicador; ?>" role="button" title="Descargar" target="_blank"><em class="fa fa-download"></em></a>
        						<?php } else if ($indicador == 2) { ?>
        						<a class="btn btn-primary" id="btnExportar" href="excelReporte.php?sede=<?php echo $_SESSION["sede"]; ?>&anhoInicio=<?php echo $anhoInicio; ?>&anhoFin=<?php echo $anhoFin; ?>&opcion=6&indicador=<?php echo $indicador; ?>" role="button" title="Descargar" target="_blank"><em class="fa fa-download"></em></a>
        						<?php } ?>
	...
	<div class="box box-primary">
		<div class="box box-success">
            <div class="box-header">
    			<div class="box-title">Reporte Tipo Descuento</div>
    		</div>
    		<div class="box-body">
...
<script type="text/javascript">
	<?php        
    	$labels = "[";
    	$keys = "[";
    	for ($indice = 0; $indice < count($lstTipoDescuento); $indice++) {
    	    $labels .= "'".strtoupper($lstTipoDescuento[$indice]->nombre)."',";
    	    $keys .= "'".$indice."',";
    	}
        ...
        if ($indicador == 0) {
            for ($i = $sd; $i <= $ed; $i += (60 * 60 * 24)) {                
                for ($indice = 0; $indice < count($lstTipoDescuento); $indice++) {
                    if ($indice == 0) {
                        $total = $lstPago[$i][0]->dscto_pedido;
                        if ($total == "") {
                            $total = 0;
                        }
                    ...
                    if ($indice == 1) {
                        $total = $lstPago[$i][0 ]->dscto_programado;
                        if ($total == "") {
                            $total = 0;
                        }
                        echo "data".($indice+1)."[c] = " . $total . ";";
