<?php
require_once("/var/www/html/ccra/cpegeneracion/sunat/funciones.php");
function run($arr, $dirxml, $dircdr, $nodo="", $tipodoc="", $enviar=false) {
    ...
    $ok = true;
    file_put_contents("info" . date("Ymd") . ".log", "Envio Electronico - Tipo Documento : " . $tipodoc . "\n", FILE_APPEND);
    if($tipodoc=='DespatchAdvice' || $tipodoc=='SummaryDocuments'){
        $xml = save($arr, $dirxml);
        file_put_contents("info" . date("Ymd") . ".log", "Envio Electronico - XML : " . implode($xml) . "\n", FILE_APPEND);
        $xml = firmar($xml, $dirxml, $arr);
        file_put_contents("info" . date("Ymd") . ".log", "Envio Electronico - XML : " . implode($xml) . "\n", FILE_APPEND);
        $signvalue = get_signaturevalue($xml);
        file_put_contents("info" . date("Ymd") . ".log", "Envio Electronico - SIGNVALUE : " . $signvalue . "\n", FILE_APPEND);
        $digvalue = get_DigestValue($xml);
        file_put_contents("info" . date("Ymd") . ".log", "Envio Electronico - DIGVALUE : " . $digvalue . "\n", FILE_APPEND);
    }else{
        $ok = valida($tipodoc);
        file_put_contents("info" . date("Ymd") . ".log", "Envio Electronico - OK : " . $ok . "\n", FILE_APPEND);
        if ($ok) {
            $xml = save($arr, $dirxml);
            file_put_contents("info" . date("Ymd") . ".log", "Envio Electronico - XML : " . implode($xml) . "\n", FILE_APPEND);
            $xml = firmar($xml, $dirxml, $arr);
    ...
    if (!$ok) {
        ...
        if($enviar){
            ...
            file_put_contents("info" . date("Ymd") . ".log", "Envio Electronico - Directorio XML : " . $dirxml . "\n", FILE_APPEND);
            file_put_contents("info" . date("Ymd") . ".log", "Envio Electronico - Nombre Archivo : " . $arr["filename"] . "\n", FILE_APPEND);
...
function genera_xml($arr, $nodo, $tipodoc) {
    ...
    if ($tipodoc == 'Invoice'){
        ...
        add_PaymentTerms($arr,$nodo,$tipodoc);
...
function add_IssueTime($arr,$nodo,$tipodoc) {
    global $root, $xml;
    $IssueTime = $xml->createElement("cbc:IssueTime", xml_hora($arr['IssueDate']));
    $IssueTime = $root->appendChild($IssueTime);
}
    ...
    $AccountingSupplierParty = $root->appendChild($AccountingSupplierParty);
        ...
        $Party = $AccountingSupplierParty->appendChild($Party);
            ...
            $PartyIdentification = $Party->appendChild($PartyIdentification);
                ...
                $ID = $xml->createElement("cbc:ID", $arr['AccountingSupplierParty']['CustomerAssignedAccountID']);
                $ID = $PartyIdentification->appendChild($ID);
                $ID->SetAttribute("schemeID", $arr['AccountingSupplierParty']['AdditionalAccountID']);
    ...
    $AccountingCustomerParty = $root->appendChild($AccountingCustomerParty);
        ...
        $Party = $AccountingCustomerParty->appendChild($Party);
            ...
            $PartyIdentification = $Party->appendChild($PartyIdentification);
                ...
                $ID = $xml->createElement("cbc:ID", $arr['AccountingCustomerParty']['CustomerAssignedAccountID']);
                $ID = $PartyIdentification->appendChild($ID);
                $ID->SetAttribute("schemeID", $arr['AccountingCustomerParty']['AdditionalAccountID']);
                $ID->SetAttribute("schemeName", "SUNAT:Identificador de Documento de Identidad");
                $ID->SetAttribute("schemeAgencyName", "PE:SUNAT");
                $ID->SetAttribute("schemeURI", "urn:pe:gob:sunat:cpe:see:gem:catalogos:catalogo06");
...
//<cac:PaymentTerms>
function add_PaymentTerms($arr,$nodo,$tipodoc){
    global $root, $xml;
    if(isset($arr['PaymentTerms']['ID']['ID'])) {
        $PaymentTerms = $xml->createElement("cac:PaymentTerms");
        $PaymentTerms = $root->appendChild($PaymentTerms);
        
        //cbc:ID
        $ID = $xml->createElement("cbc:ID", $arr['PaymentTerms']['ID']['ID']);
        $ID = $PaymentTerms->appendChild($ID);
        //cbc:PaymentMeansID
        $PaymentMeansID = $xml->createElement("cbc:PaymentMeansID", $arr['PaymentTerms']['PaymentMeansID']['PaymentMeansID']);
        ...
        $InvoiceLine = $root->appendChild($InvoiceLine);
            ...
            for ($ii=1; $ii<=sizeof($arr['InvoiceLine'][$i]['PricingReference']['AlternativeConditionPrice']); $ii++) {
                ...
                $AlternativeConditionPrice = $PricingReference->appendChild($AlternativeConditionPrice);
                    ...
                    $PriceTypeCode->setAttribute("SUNAT:Indicador de Tipo de Precio");
            ...
            for ($iii=1; $iii<=sizeof($arr['InvoiceLine'][$i]['TaxTotal']); $iii++) {
                ...
                $TaxTotal = $InvoiceLine->appendChild($TaxTotal);
                    ...
                    $TaxSubtotal = $TaxTotal->appendChild($TaxSubtotal);
                        ...
                        $TaxableAmount = $xml->createElement("cbc:TaxableAmount", $arr['InvoiceLine'][$i]['LineExtensionAmount']['Amount']);
                        $TaxableAmount = $TaxSubtotal->appendChild($TaxableAmount);
                        $TaxableAmount->SetAttribute("currencyID", $arr['InvoiceLine'][$i]['TaxTotal'][$iii]['TaxSubtotal']['TaxAmount']['currencyID']);
                        ...
                        $TaxCategory = $TaxSubtotal->appendChild($TaxCategory);
                            ...
                            //<cac:ID>
                            if($arr['InvoiceLine'][$i]['TaxTotal'][$iii]['TaxSubtotal']['TaxCategory']['TaxExemptionReasonCode'] == "10"){
                            ...
                            $Percent = "";
                            if($arr['InvoiceLine'][$i]['TaxTotal'][$iii]['TaxSubtotal']['TaxCategory']['TaxExemptionReasonCode'] == "10"){
                                $Percent = $xml->createElement("cbc:Percent", "18.00");
...
function add_TaxTotal($arr,$nodo,$tipodoc){
    ...
    $TaxTotal = $root->appendChild($TaxTotal);
        ...
        //<cbc:TaxAmount
        $TaxAmount = $xml->createElement("cbc:TaxAmount", $arr['TaxTotal'][1]['TaxAmount']['Amount']);
        $TaxAmount = $TaxTotal->appendChild($TaxAmount);
        $TaxAmount->SetAttribute("currencyID", $arr['TaxTotal'][1]['TaxAmount']['currencyID']);
        

        for ($i=1; $i<=sizeof($arr['TaxTotal']); $i++) {
            //CREAR MAS DE UNO SI ES QUE HAY MAS DE UN TIPO DE IGV
            //<cac:TaxSubtotal>
            $TaxSubtotal = $xml->createElement("cac:TaxSubtotal");
            $TaxSubtotal = $TaxTotal->appendChild($TaxSubtotal);
                    
                //cbc:TaxableAmount
                $TaxableAmount = $xml->createElement("cbc:TaxableAmount", $arr['TaxTotal'][$i]['TaxSubtotal']['TaxableAmount']['Amount']);
                $TaxableAmount = $TaxSubtotal->appendChild($TaxableAmount);
                $TaxableAmount->SetAttribute("currencyID", $arr['TaxTotal'][$i]['TaxSubtotal']['TaxAmount']['currencyID']);
                ...
                $TaxCategory = $TaxSubtotal->appendChild($TaxCategory);
                    ...
                    //<cbc:ID>
                    $ID = $xml->createElement("cbc:ID", $arr['TaxTotal'][$i]['TaxSubtotal']['TaxCategory']['ID']);
                    $ID = $TaxCategory->appendChild($ID);
                    $ID->SetAttribute("schemeID", "UN/ECE 5305");
                    $ID->SetAttribute("schemeName", "Tax Category Identifier");
                    $ID->SetAttribute("schemeAgencyName", "United Nations Economic Commission for Europe");
...
function add_LegalMonetaryTotal($arr,$nodo,$tipodoc){
    ...
    $LegalMonetaryTotal = $root->appendChild($LegalMonetaryTotal);
        //<cbc:LineExtensionAmount
        if(isset($arr['LegalMonetaryTotal']['LineExtensionAmount'])){
            $LineExtensionAmount = $xml->createElement("cbc:LineExtensionAmount", $arr['LegalMonetaryTotal']['LineExtensionAmount']['TotalAmount']);
            $LineExtensionAmount = $LegalMonetaryTotal->appendChild($LineExtensionAmount);
            $LineExtensionAmount->SetAttribute("currencyID", $arr['LegalMonetaryTotal']['LineExtensionAmount']['currencyID']);
        }
        //<cbc:TaxInclusiveAmount
        if(isset($arr['LegalMonetaryTotal']['TaxInclusiveAmount'])){
            $TaxInclusiveAmount = $xml->createElement("cbc:TaxInclusiveAmount", $arr['LegalMonetaryTotal']['TaxInclusiveAmount']['TotalAmount']);
            $TaxInclusiveAmount = $LegalMonetaryTotal->appendChild($TaxInclusiveAmount);
            $TaxInclusiveAmount->SetAttribute("currencyID", $arr['LegalMonetaryTotal']['TaxInclusiveAmount']['currencyID']);
        }
        ...
        if(isset($arr['LegalMonetaryTotal']['PayableAmount'])){
            if ($arr['TaxTotal'][1]['TaxSubtotal']['TaxAmount']['Amount'] != 0) {
...
function add_LineCountNumeric($arr,$nodo,$tipodoc){
    global $root, $xml;
    $LineCountNumeric = $xml->createElement("cbc:LineCountNumeric", $arr['LineCount']);
    $LineCountNumeric = $root->appendChild($LineCountNumeric);
}
            ...
            for ($iii=1; $iii<=sizeof($arr['CreditNoteLine'][$i]['TaxTotal']); $iii++) {
                ...
                $TaxTotal = $CreditNoteLine->appendChild($TaxTotal);
                    ...
                    $TaxSubtotal = $TaxTotal->appendChild($TaxSubtotal);
                        ...
                        //cbc:TaxableAmount
                        //$TaxableAmount = $xml->createElement("cbc:TaxableAmount", $arr['CreditNoteLine'][$i]['TaxableAmount']['Amount']);
                        //$TaxableAmount = $TaxSubtotal->appendChild($TaxableAmount);
                        //$TaxableAmount->SetAttribute("currencyID", $arr['CreditNoteLine'][$i]['TaxableAmount']['currencyID']);

                        $TaxableAmount = $xml->createElement("cbc:TaxableAmount", $arr['CreditNoteLine'][$i]['LineExtensionAmount']['Amount']);
                        $TaxableAmount = $TaxSubtotal->appendChild($TaxableAmount);
                        $TaxableAmount->SetAttribute("currencyID", $arr['CreditNoteLine'][$i]['TaxTotal'][$iii]['TaxSubtotal']['TaxAmount']['currencyID']);
